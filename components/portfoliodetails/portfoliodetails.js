...
const Portfoliodetails = () => {
  ...
  async function getUser() {
    try {
      const response = await axios.get('http://localhost:1337/api/profiles/?populate=%2A');
      ...
      <><div class="max-w-xs mx-9 mb-2">
        <div className="cards">
          ...
          <header class="coloumn items-center justify-between leading-tight p-2 md:p-4">
            <h1 class="text-lg">
              <span class="no-underline hover:underline text-black" href="#">
                {itms.attributes.first_name} {itms.attributes.last_name}
              </span>
            </h1>
            <p
          ...
          <footer class="card flex items-center justify-evenly leading-none p-4 md:p-4">
            ...
            <p style={{ color: "rgba(30, 30, 30, 0.5)" }}> {itms.attributes.star}</p>
            <svg
              width="2"
              height="23"
              viewBox="0 0 2 23"
            ...
            <p style={{ color: "rgba(30, 30, 30, 0.5)" }}> {itms.attributes.marriage_status}</p>
          </footer>
        </div>
      </div>
