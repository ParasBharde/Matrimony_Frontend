...
const EditProfileForm3 = ({ screen, setScreen, userData }) => {
  ...
  useEffect(() => {
    if(userData) {
      ...
      setFatherNative(userData.father_native)
      setMotherNative(userData.mother_native)

      setFatherProfession(userData.father_profession)
      setMotherProfession(userData.mother_profession)

      setPhoneNumber(userData.parents_contact_number)
      setAddress(userData.address)

      setFather(userData.Father)
      setMother(userData.Mother)

      setBrothers(userData.brothers)
      setYoungerBrothers(userData.younger_brothers)
      setElderBrothers(userData.elder_brothers)
      setMarriedBrothers(userData.married_brothers)

      setSisters(userData.sisters)
      setYoungerSisters(userData.younger_sisters)
      setElderSisters(userData.elder_sisters)
      setMarriedSisters(userData.married_sisters)
    }
  },[userData])

  useEffect(() => {
    const edit = sessionStorage.getItem("edit3")
    if (edit) {
  ...
  const beforeNextScreen = () => {
    const edit3 = { fatherName, motherName, fatherNative, motherNative, fatherProfession, motherProfession, phoneNumber, address, father, mother, brothers, youngerBrothers, elderBrothers, marriedBrothers, sisters, youngerSisters, elderSisters, marriedSisters }
    sessionStorage.setItem("edit3", JSON.stringify(edit3))
  }
  ...
  return (
    <>
      ...
      <div className=" w-[820px] mx-auto max-md:flex max-md:gap-5 max-md:w-auto max-md:m-3">
...
export default EditProfileForm3